...
function MasterDbV({onBack,onLogout,master,setMaster,t_,role}){
  ...
  const [tab,setTab]=useState("list");
  const [confirm,setConfirm]=useState(null);
  const [form,setForm]=useState({plate:"",name:"",dept:"",type:"official",contact:""});
  const importRef=useRef();

  const exportXL=()=>{
    if(!master.length){t_("No data","amber");return;}
    const ws=XLSX.utils.json_to_sheet(master.map(r=>({
      "Plate Number":r.plate||"","Type":r.type||"","Name":r.name||r.officer||"",
      "Department":r.dept||r.division||"","Contact":r.contact||"",
      "First Seen":r.firstSeen||"","Last Seen":r.lastSeen||""
    })));
    const wb=XLSX.utils.book_new();
  ...
  const addRecord=async()=>{
    if(!form.plate.trim()){t_("Plate number is required","red");return;}
    const p=norm(form.plate);
    if((master||[]).find(m=>norm(m.plate||"")===p)){t_("Plate already exists in master","amber");return;}
    const rec={plate:p,type:form.type||"official",name:form.name.trim(),dept:form.dept.trim(),contact:form.contact.trim(),firstSeen:fmtDate(Date.now()),lastSeen:fmtDate(Date.now())};
    const nm=[...(master||[]),rec];setMaster(nm);await ss(K.master,nm);
    setForm({plate:"",name:"",dept:"",type:"official",contact:""});
  ...
  const importXL=ev=>{
    ...
    reader.onload=e=>{
      try{
        ...
        rows.forEach(row=>{
          const plate=getCol(row,["plate number","plate","car number","vehicle number","reg number","registration"]);
          if(!plate)return;
          const p=norm(plate);
          if(cur.find(r=>norm(r.plate||"")===p))return;
          cur.push({
            plate:p,
            type:getCol(row,["type"])||"official",
            name:getCol(row,["name","name/officer","officer name","official name","visitor name"]),
            dept:getCol(row,["department","dept","department/division","division"]),
    ...
    <div style={{padding:"14px 15px 0"}}>
      <Card>
        ...
        <FormField label="Type">
          <select value={form.type} onChange={e=>setForm(p=>({...p,type:e.target.value}))} style={{background:"rgba(255,255,255,0.05)",border:`1px solid ${T.border}`,borderRadius:9,color:"white",padding:"11px 13px",fontSize:14,width:"100%"}}>
            {["official","visitor","come&play","exception"].map(o=><option key={o} value={o}>{o}</option>)}
          </select>
        </FormField>
        <FormField label="Name / Officer"><input placeholder="Full name" value={form.name} onChange={e=>setForm(p=>({...p,name:e.target.value}))}/></FormField>
        <FormField label="Department"><input placeholder="e.g. Administration, Security" value={form.dept} onChange={e=>setForm(p=>({...p,dept:e.target.value}))}/></FormField>
  ...
  if(tab==="import") return <div style={{minHeight:"100vh",background:T.navy}}>
    ...
    <div style={{padding:"14px 15px 0"}}>
      <div style={{background:"rgba(37,99,235,0.07)",border:"1px solid rgba(37,99,235,0.25)",borderRadius:10,padding:"12px 14px",marginBottom:14,fontSize:11,color:T.blueT,lineHeight:1.9}}>
        <b>Expected Excel columns:</b><br/>
        <span style={{fontFamily:"monospace",color:T.goldL}}>Plate Number</span> · <span style={{fontFamily:"monospace",color:T.goldL}}>Type</span> · <span style={{fontFamily:"monospace",color:T.goldL}}>Name</span> · <span style={{fontFamily:"monospace",color:T.goldL}}>Department</span> · <span style={{fontFamily:"monospace",color:T.goldL}}>Contact</span><br/>
        <span style={{color:"rgba(255,255,255,0.4)"}}>Duplicate plates are automatically skipped.</span>
      </div>
      <div onClick={()=>importRef.current?.click()} style={{display:"flex",flexDirection:"column",alignItems:"center",gap:10,padding:"40px 32px",border:"2px dashed rgba(37,99,235,0.4)",borderRadius:12,cursor:"pointer",background:"rgba(37,99,235,0.04)"}}>
        <span style={{fontSize:40}}>📂</span>
        <div style={{fontSize:14,fontWeight:600,color:"rgba(255,255,255,0.85)"}}>Tap to Select Excel File</div>
    ...
    <div style={{padding:"14px 15px 0"}}>
      ...
      {!filtered.length
        ...
        :filtered.slice().reverse().map((r,i)=>(
          <div key={r.plate||i} style={{background:"rgba(255,255,255,0.03)",border:`1px solid ${T.border}`,borderLeft:`2px solid ${T[typeColor[r.type||""]||"green"]||T.green}`,borderRadius:11,padding:"11px 13px",marginBottom:7,display:"flex",alignItems:"center",gap:10}}>
            <div style={{flex:1}}>
