...
function StaffApp({onLogout,master,setMaster,officials,visitors,setVisitors,cnp,log,setLog,banned,passes,setPasses,user}){
  ...
  const doAction=action=>{
    ...
    if(action==="entry"){
      ...
      if(vehicle&&(vehicle.type==="visitor"||vehicle.type==="cnp")){
...
function AdminApp({onLogout,master,setMaster,officials,setOfficials,visitors,setVisitors,cnp,setCnp,log,setLog,users,setUsers,banned,setBanned,passes,setPasses,user}){
  ...
  const OFFICIAL_FIELDS=[
    {key:"officialName",label:"Name of Official",required:true,isKey:false},
  ...
  const downloadAllExcel=()=>{
    ...
    if(officials.length){const ws=XLSX.utils.json_to_sheet(officials.map(r=>({"Name":r.officialName,"Car Number":r.carNumber,"Department":r.department,"Entry Time":r.entryTime||"","Exit Time":r.exitTime||""})));XLSX.utils.book_append_sheet(wb,ws,"Officials");}
