...
function MasterDbV({onBack,onLogout,master,t_,role}){
  ...
  const filtered=safeSearch
    ?master.filter(r=>
        norm(r.plate||"").includes(norm(search))||
        (r.name||"").toLowerCase().includes(safeSearch)||
        (r.dept||"").toLowerCase().includes(safeSearch)||
        (r.type||"").toLowerCase().includes(safeSearch)
      )
    :master;
  ...
  return <div style={{minHeight:"100vh",background:T.navy}}>
    ...
    <div style={{padding:"14px 15px 0"}}>
      <div style={{background:"rgba(232,160,32,0.07)",border:"1px solid rgba(232,160,32,0.2)",borderRadius:9,padding:"10px 14px",marginBottom:12,fontSize:11,color:T.amberT,lineHeight:1.6}}>
        📋 Auto-updated on every vehicle entry. All vehicles that have ever visited JLN Stadium.
      </div>
      ...
      {search&&<div style={{fontSize:10,color:T.muted,marginBottom:8}}>{filtered.length} result{filtered.length!==1?"s":""}</div>}
      {!filtered.length
        ?<div style={{textAlign:"center",color:T.muted,padding:"40px 0",fontSize:12}}>{search?"No vehicles match":"No vehicles yet"}</div>
        :filtered.slice().reverse().map((r,i)=>(
          <div key={r.plate||i} style={{background:"rgba(255,255,255,0.03)",border:`1px solid ${T.border}`,borderLeft:`2px solid ${T[typeColor[r.type]||"green"]||T.green}`,borderRadius:11,padding:"11px 13px",marginBottom:7}}>
