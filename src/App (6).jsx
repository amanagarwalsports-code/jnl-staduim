...
function StaffApp({onLogout,master,log,setLog,banned,passes,setPasses,cnp,user}){
  ...
  const addException=()=>{
    ...
    const now=Date.now();
    const entry={id:now,plate:plate.toUpperCase(),type:"guest",entryTime:now,exitTime:null,date:todayStr(),guestName:guestName.trim(),guestReason:guestReason.trim(),officer:user.username,division:"Exemption",loggedBy:user.username};
    const nl=[...log,entry];setLog(nl);ss(K.log,nl);
    ...
    setResult({status:"allowed_guest",plate:plate.toUpperCase(),detail:`Guest: ${entry.guestName} · ${entry.guestReason}`});
  };
  ...
  if(view==="exception") return <div style={{minHeight:600,background:T.navy}}>
    ...
    <div style={{padding:"15px 15px 0"}}>
      <div style={{background:"rgba(217,119,6,0.08)",border:`1px solid rgba(217,119,6,0.3)`,borderRadius:12,padding:"14px 15px",marginBottom:14}}>
        <div style={{fontFamily:"'Barlow Condensed'",fontSize:16,fontWeight:800,color:T.amberT,letterSpacing:.5}}>Exception / Exemption Entry</div>
        <div style={{fontSize:11,color:"rgba(253,211,77,0.6)",marginTop:4}}>Fill the 3 required fields below to grant one-time entry</div>
      </div>
      <Card>
        <SecTitle>REQUIRED DETAILS</SecTitle>
        <FormField label="Vehicle Registration Number" required>
          <input placeholder="DL 01 AB 1234" value={plate} onChange={e=>setPlate(e.target.value.toUpperCase())} style={{fontFamily:"'Barlow Condensed'",fontSize:22,fontWeight:800,letterSpacing:4,textAlign:"center"}}/>
        </FormField>
  ...
  return <div style={{minHeight:600,background:T.navy}}>
    ...
    <div style={{padding:"16px 15px 0"}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:9,marginBottom:9}}>
        <div onClick={()=>setView("passes")} style={{background:"rgba(8,145,178,0.06)",border:"1px solid rgba(8,145,178,0.25)",borderRadius:12,padding:"12px 14px",cursor:"pointer",display:"flex",alignItems:"center",gap:10}}>
          <div style={{width:32,height:32,borderRadius:8,background:"rgba(8,145,178,0.12)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:14,flexShrink:0}}>🎫</div>
          <div style={{flex:1}}>
            <div style={{fontFamily:"'Barlow Condensed'",fontSize:13,fontWeight:700,color:T.cyanT,letterSpacing:.3}}>Visitor Passes</div>
            <div style={{fontSize:9,color:T.muted,marginTop:1}}>{approvedPasses.length} approved</div>
          </div>
        </div>
        <div onClick={()=>{setPlate("");setLast4("");setSuggestions([]);setResult(null);setGuestName("");setOfficer("");setDivision("");setGuestReason("");setRefOfficerName("");setRefOfficerDesig("");setRefOfficerDept("");setRefOfficerPhone("");setView("exception");}} style={{background:"rgba(217,119,6,0.06)",border:"1px solid rgba(217,119,6,0.25)",borderRadius:12,padding:"12px 14px",cursor:"pointer",display:"flex",alignItems:"center",gap:10}}>
          <div style={{width:32,height:32,borderRadius:8,background:"rgba(217,119,6,0.12)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:14,flexShrink:0}}>⚠</div>
          <div style={{flex:1}}>
            <div style={{fontFamily:"'Barlow Condensed'",fontSize:13,fontWeight:700,color:T.amberT,letterSpacing:.3}}>Grant Exemption</div>
            <div style={{fontSize:9,color:T.muted,marginTop:1}}>One-time exception entry</div>
          </div>
        </div>
      </div>
    </div>
