  ...
  return <div style={{minHeight:"100vh",background:T.navy}}>
    ...
    <div style={{padding:"16px 15px 0"}}>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:9,marginBottom:16}}>
        {[
          {label:"Inside Now",value:inside,icon:"🚗",color:T.goldL,action:"carlog"},
          {label:"Today's Entries",value:todayLog.length,icon:"📋",color:T.blueT,action:"carlog"},
          {label:"Officials",value:safeOfficials.length,icon:"◆",color:T.amberT,action:"officials"},
          {label:"Visitors",value:safeVisitors.length,icon:"🎫",color:T.cyanT,action:"visitors"}
        ].map(s=>(
          <div key={s.label} onClick={()=>setView(s.action)} style={{background:"rgba(255,255,255,0.03)",border:`1px solid ${T.border}`,borderRadius:12,padding:"14px 13px",cursor:"pointer",transition:"background .15s"}}
            ...
            <div style={{fontSize:9,color:"rgba(255,255,255,0.2)",marginTop:3}}>Tap to view →</div>
          </div>
        ))}
      </div>
      ...
      {[
        ...
        {icon:"🚗",label:"Car Log",sub:`${safeLog.length} total entries · ${inside} inside now`,action:"carlog",color:T.blue},
